...
5. Click **Connect ESP32** (Section 2) to open the serial connection.
6. Collect images, train, export weights. Done.
...
For each class:
1. Name the class in the text field (or sync from `config.json`)
2. Position your object under your deployment lighting
3. Click **📷 Capture** to add a frame to the buffer
4. Repeat until you have 15–40 samples per class
...
3. Update the three `<script src="...">` lines at the top of `index.html` to local paths
...
|---|---|---|
| [on-device-vision-ai](https://github.com/webmcu-ai/on-device-vision-ai) | Paper 1 | C++ on-device CNN training and inference firmware |
| [webmcu-vision-web](https://github.com/webmcu-ai/webmcu-vision-web) | Paper 2 | This repository — browser companion |
...
*By [Jeremy Ellis](https://github.com/hpssjellis) · High School Robotics Educator, British Columbia, Canada*
